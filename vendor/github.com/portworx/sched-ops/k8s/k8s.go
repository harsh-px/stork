...
// WatchFunc is a callback provided to the WatchNode function
// which is invoked when the v1.Node object is changed.
type WatchFunc func(object runtime.Object) error

// ConfigMapWatchFunc is a callback which is invoked when the v1.ConfigMap object is changed.
//type ConfigMapWatchFunc func(cm *v1.ConfigMap) error
		...
		case event, more := <-watchInterface.ResultChan():
			if !more {
				...
				if _, err := task.DoRetryWithTimeout(t, 10*time.Minute, 10*time.Second); err != nil {
					logrus.WithError(err).Error("Could not reestablish the watch")
				} else {
...
func (k *k8sOps) WatchConfigMap(configMap *v1.ConfigMap, fn WatchFunc) error {
	...
	if err != nil {
		logrus.Errorf("[debug] error invoking the watch api for config maps: %v", err)
		return err
	}
